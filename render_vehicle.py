...
# Vehicle camera definitions (from road_car_transfer)
VEHICLE_CAMERAS = {
...
def load_world2lidar(transform_json_path, timestamp_ms):
    ...
    Args:
        transform_json_path: path to JSON with world2lidar entries
    ...
    for entry in transforms:
        ts = entry['timestamp']
        # timestamps may be in seconds (float) or milliseconds (int)
...
def compute_vehicle_cam_pose(R_w2l, t_w2l, R_cam2lidar, t_cam2lidar):
    ...
    Returns:
        ...
        (R_stored = w2c_rotation transposed, T_stored = w2c_translation)
    """
    ...
    # DNGaussian convention: R is stored transposed
...
def create_vehicle_camera(R_stored, T_stored, fovx, fovy, width, height, cam_name, uid):
    """Create a DNGaussian Camera object for a vehicle camera viewpoint."""
    cam = Camera(
        colmap_id=uid,
        R=R_stored,
        T=T_stored,
...
def render_vehicle_cameras(model_path, vehicle_calib, transform_json, timestamp_ms,
    ...
    checkpoint_path = os.path.join(model_path, "chkpnt_latest.pth")
    ...
    for cam_id in tqdm(camera_ids, desc="Vehicle cameras"):
        ...
        cam = create_vehicle_camera(
            ...
            cam_name=f"vehicle_{cam_name}", uid=cam_id
        )

        # Render
        with torch.no_grad():
            render_pkg = render(cam, gaussians, pipeline, bg_color, inference=True)
        ...
        depth_vis = depth_vis + 1.0 * (1 - alpha)
        ...
        tqdm.write(f"  {cam_name}: rendered {w}x{h}, fov={math.degrees(fovx):.1f}x{math.degrees(fovy):.1f}°")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--vehicle_calib", type=str, required=True,
                        help="Path to vehicle calibration folder (camera_XX_intrinsics.yaml etc.)")
